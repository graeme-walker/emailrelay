...
{
	return FactoryParser::check( address , "net" ) ;
}
...
GSmtp::Verifier * GSmtp::VerifierFactory::newVerifier( const std::string & address , unsigned int timeout )
{
	...
	{
		return new NetworkVerifier( p.second , timeout , timeout ) ;
	}
	else
	{
		return new ExecutableVerifier( G::Executable(p.second) ) ;
	}
}

/// \file gverifierfactory_exec_enabled.cpp
